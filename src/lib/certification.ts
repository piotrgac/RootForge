...
export const certificationRoadmaps: Roadmap[] = [
  {
    name: 'LPIC-1',
...
export function getRoadmapByName(name: string): Roadmap | null {
  return certificationRoadmaps.find(r => r.name === name) || null;
}

export function getTopicsByCategory(category: string): (Topic & { cert: string })[] {
  const result: (Topic & { cert: string })[] = [];
